            ...
            >
              <div 
                className={`p-3 rounded border ${
                  formData.questionType === type.value ? 'border-primary bg-light' : 
                  type.disabled ? 'text-muted opacity-50' : ''
                }`}
                style={{ 
                  cursor: type.disabled ? 'not-allowed' : 'pointer',
                  height: '100%'
                }}
                onClick={() => !type.disabled && updateFormData('questionType', type.value)}
              >
                <Form.Check
                  type="radio"
                  id={`qtype-${type.value}`}
