...
def sync_mock_pdfs():
    """
    Synchronizes PDF files referenced in client mockModules.js with the uploads folder.
    This ensures that even after clearing cache, the PDFs remain available.
    """
    try:
        # Default mock PDF filenames from the client mockModules.js
        mock_pdf_files = [
            "ode-til-doden.pdf",
            "kreativ-skrivning.pdf",
            "litteraer-analyse.pdf",
            "postmodernisme.pdf",
            "filmanalyse.pdf",
            "fortaelleteknik.pdf",
            "essay-kendetegn.pdf",
            "essay-eksempel.pdf",
            "retorik.pdf",
            "projekt-beskrivelse.pdf",
            "afleverings-guide.pdf"
        ]
        
        # Import reportlab only when needed to avoid dependency issues
        try:
            from reportlab.pdfgen import canvas
            has_reportlab = True
        except ImportError:
            logger.warning("ReportLab not installed. Mock PDFs will use empty files instead.")
            has_reportlab = False
        
        results = []
        
        for filename in mock_pdf_files:
            filepath = os.path.join(UPLOAD_FOLDER, filename)
            
            # Only create if file doesn't exist
            if not os.path.exists(filepath):
                if has_reportlab:
                    # Create a simple placeholder PDF with reportlab
                    c = canvas.Canvas(filepath)
                    c.setFont("Helvetica", 24)
                    c.drawString(100, 700, "Placeholder for: " + filename)
                    c.setFont("Helvetica", 18)
                    c.drawString(100, 650, "This file is automatically generated as a placeholder")
                    c.drawString(100, 620, "for a file referenced in the mock data.")
                    c.save()
                else:
        ...
        return jsonify({
            "success": True,
            "message": f"Synchronized {len(results)} mock PDF files",
            "files": results
        })
    
    except Exception as e:
        logger.error(f"Error syncing mock PDFs: {str(e)}")
        return jsonify({
            "success": False,
...
if __name__ == "__main__":
    # Before starting, ensure mock PDFs are synced
    try:
        logger.info("Ensuring mock PDFs are available...")
        # Manually simulate a request to sync mock PDFs 
        with app.test_client() as client:
            client.get('/api/sync-mock-pdfs')
    except ImportError:
        logger.warning("ReportLab not installed. Mock PDFs will not be automatically created.")
    except Exception as e:
